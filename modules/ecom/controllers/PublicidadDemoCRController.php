<?php
class PublicidadDemoCRController extends Controller{
  ...
  public function actionEditElement(){
    $this->template = null;
    $data = new PublicidadDemoCR($this->params["id"]);
    $this->render("edit", array("data" => $data));
  }

  public function actionGetAll(){
    $template = null;
    $rows = PublicidadDemoCR::model()->findAll("WHERE estatus = 1");
    ...
    public function actionSave(){
      $model = new PublicidadDemoCR($this->params["id"]);
      ...
      if(!$model->save()){
        ...
        $model = new PublicidadDemoCR($dataModel->id);
      } 
      
      // Si se agregó la imagen para el sabor, la cargamos
      if(isset($_FILES["portada"]) && $this->error == ""){
        $dir_subida = Motor::app()->absolute_url.$this->murl."/images/publicidaddemocr/";
        if(!file_exists($dir_subida)){
          mkdir($dir_subida, 0775);
        }
        $path = $_FILES['portada']['name'];
        $ext = pathinfo($path, PATHINFO_EXTENSION);
        $fichero_guardado = "images/publicidaddemocr/".($dataModel->id)."_portada_.".$ext;
  ...
  public function actionFinalizaPublicidadDemoCR(){
    $model = new PublicidadDemoCR($this->params["id"]);
    if($model->id == ""){
  ...
  public function actionSavePermisos(){
    ...
    $model = new PublicidadDemoCR($rolId);
