<?php
  class PublicidadPautaCRController extends Controller{
    ...
    public function actionGetAll(){
      ...
      $rows = PublicidadPautaCR::model()->findAll($auxWhere);
      $tienePermiso = "0";
      if($this->revisaPermiso("PublicidadPautaCR", "ModificaPaquetePlaza", $this->user->rol)){
        $tienePermiso = "1";
      }
      foreach ($rows as $key => $value) {
        ...
        $cuenta = new PublicidadPaqueteCuenta($value->cuenta);
        $rows[$key]->cuenta_nombre = $cuenta->nombre;
    ...
    public function actionGetAllPublicaciones(){
      ...
      if($this->user->id == 1){
        ...
        $auxWhere = " WHERE plaza IN (".$objReclutador->plazas.") ";
      }
      $tipos = array(
        "1" => "Sencilla",
        "2" => "Doble",
    ...
    public function actionGetAllPublicacionesReporte(){
      $this->template = null;
      $rows = PublicidadPautaCRPublicacion::model()->findAll("WHERE fecha IS NOT NULL AND fecha != '0000-00-00'");
      foreach ($rows as $key => $value) {
        $pp = new PublicidadPautaCR($value->paquete_plaza);
    ...
    public function actionEditElement(){
      $this->template = null;
      $data = new PublicidadPautaCR($this->params["id"]);
      $this->render("edit", array("data"=>$data));
    }

    public function actionEditPublicacion(){
      $this->template = null;
      $data = new PublicidadPautaCRPublicacion($this->params["id"]);
    ...
    public function actionEditPublicacionNV2(){
      $data = new PublicidadPautaCR($this->params["id"]);
      $this->render("edit-pauta-nv2", array("data"=>$data));
    }
    ...
    public function actionSave(){
      $fecha = date('Y-m', strtotime("+1 month", time()))."-01";
	     //$fecha = "2023-09-01";	
      $actual = new PublicidadPautaCR("WHERE plaza = ".$this->params["plaza"]." AND fecha = '$fecha'");
      if($actual->id != ""){
      ...
      if($this->error == ""){
        ...
        if($paquete->publicaciones > 0){
          for ($i=0; $i < $paquete->publicaciones; $i++) { 
            ...
            $amodel = new PublicidadPautaCRPublicacion();
    ...
    public function actionDestroy(){
      $this->template = null;
      $model = new PublicidadPautaCR($this->params["id"]);
      $model->estatus = 2;
      if(!$model->save()){
    ...
    public function actionSavePublicacion(){
      ...
      $previas = PublicidadPautaCR::model()->findAll("WHERE plaza = ".$this->params["plaza"]." AND user = ".$this->user->id);
      ...
      if($this->error == ""){
        ...
        $problema = new PublicidadPautaCR("WHERE plaza = ".$this->params["plaza"]." AND (fecha BETWEEN '$fecha_inicio' AND '$fecha_fin' OR fecha_fin BETWEEN '$fecha_inicio' AND '$fecha_fin')");
      ...
      if($this->error == ""){
        if($this->params["cuenta"] == "" || $this->params["cuenta"] == "0"){
          $this->error .= "Debes seleccionar una cuenta.";
        }
      }
    ...
    public function actionGetColores(){
      $this->template = null;
      $rows = PublicidadPaqueteColorCR::model()->findAll("WHERE diseno = ".$this->params["diseno"]);
    ...
    public function actionGetPlazas(){
      $this->template = null;
      $rows = Plaza::model()->findAll("WHERE cuenta_cr = ".$this->params["cuenta"]);
      $sHtml = '<option value="">Selecciona una opción...</option>';
      //$this->error = $this->params;
      foreach ($rows as $key => $value) {
        //$checked = (in_array($value->id, $municipiosPrevios)) ? "selected":"";
    ...
    public function actionGetPerfiles(){
      $this->template = null;
      $rows = PublicidadPaquetePerfilCR::model()->findAll("WHERE diseno = ".$this->params["diseno"]);
      $sHtml = '<option value="">Selecciona una opción...</option>';
      //$this->error = $this->params;
      foreach ($rows as $key => $value) {
        //$checked = (in_array($value->id, $municipiosPrevios)) ? "selected":"";
