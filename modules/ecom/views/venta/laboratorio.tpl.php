...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    var tableData = $('#tabla-data').DataTable({
      ...
      "serverSide": true,
      "ajax": {
        "url": "<?=$url("ecom/".$this->interfaz."/getAllPorEstatus?e=2")?>",
        "type" : "post"
      },
      "columns": [
        {"data": "id"},
        ...
        {"data": "estatus_descripcion"},
        {
          "data": function(row){
            var sHtml = "";
            sHtml += "&nbsp;<button class=\"act-btn boton-recibe\">recepcionar</button>";
            return sHtml;
          }
        },
        {
    ...
    $('#tabla-data tbody').on( 'click', 'button', function () {
      ...
      if(button.hasClass("boton-recibe")){
        modificaModel(data.id, "<?=$url("ecom/".$this->interfaz."/saveEstatus?estatus=3")?>", [], hDefault);
      }
    });
