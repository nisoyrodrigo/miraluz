...
$temporales = ProductoTemporalEdit::model()->executeQuery("DELETE FROM ec_producto_temporal_edit WHERE user = ".$this->user->id);
$detalle = VentaDetalle::model()->findAll("WHERE venta = ".$data->id);
foreach ($detalle as $key => $value) {
  $model = new ProductoTemporalEdit();
  $model->user = $this->user->id;
  $model->producto = $value->producto;
...
<div class="wrapper wrapper-content animated fadeInRight">
  <div class="row cotizacion-bg">
    ...
    <div class="col-lg-12">
        ...
        <input type="hidden" id="ventaClienteId" value="<?=$cliente->id?>">

        <div class="form-group row">
          <div class="col-lg-10">
            <input type="text" class="form-control" id="nombreCliente" readonly value="<?=$cliente->nombre?>">
          </div>
          <div class="col-lg-2">
        ...
        <div class="row">
          <div class="col-3">
            <label><input type="radio" name="tipo_vision" value="vision_sencilla" checked onchange="actualizarGraduacion()" <?=($data->tipo_vision == "vision_sencilla") ? "checked":"";?>> Visión sencilla</label>
          </div>
          <div class="col-3">
            <label><input type="radio" name="tipo_vision" value="flap_top" onchange="actualizarGraduacion()" <?=($data->tipo_vision == "flap_top") ? "checked":"";?>> Flap top</label>
          </div>
          <div class="col-3">
            <label><input type="radio" name="tipo_vision" value="blend" onchange="actualizarGraduacion()" <?=($data->tipo_vision == "blend") ? "checked":"";?>> Blend</label>
          </div>
          <div class="col-3">
            <label><input type="radio" name="tipo_vision" value="progresivo" onchange="actualizarGraduacion()" <?=($data->tipo_vision == "progresivo") ? "checked":"";?>> Progresivo</label>
          </div>
        </div>
        ...
        <div class="row">
          <div class="col-9">
                ...
                <tr>
                  ...
                  <td>
                    ...
                    <?php foreach ($graduaciones['esfera'] as $value): 
                      ...
                      ?>
                      <option value="<?= number_format($value, 2) ?>" <?= $value == $data->od_esfera ? 'selected' : '' ?>><?= $formattedValue ?></option>
                    <?php endforeach; ?>
                    </select>
                  </td>
                  <td>
                    ...
                    <?php foreach ($graduaciones['cilindro'] as $value): ?>
                      <option value="<?= number_format($value, 2) ?>" <?= $value == $data->od_cilindro ? 'selected' : '' ?>><?= number_format($value, 2) ?></option>
                    <?php endforeach; ?>
                    </select>
                  </td>
                  <td>
                    ...
                    <?php foreach ($graduaciones['eje'] as $value): ?>
                      <option value="<?= number_format($value, 0) ?>" <?= $value == $data->od_eje ? 'selected' : '' ?>><?= number_format($value, 0) ?>°</option>
                    <?php endforeach; ?>
                    </select>
                  </td>
                  <td class="col-adicion">
                    ...
                    <?php foreach ($graduaciones['add'] as $value): ?>
                      <option value="<?= number_format($value, 2) ?>" <?= $value == $data->od_add ? 'selected' : '' ?>><?= number_format($value, 2) ?></option>
                    <?php endforeach; ?>
                    </select>
                  </td>
                  <td><input type="text" class="form-control" name="dnp_od" value="<?=$data->od_dnp?>"></td>
                  <td class="col-altura"><input type="text" class="form-control" name="altura_od" value="<?=$data->od_altura?>"></td>
                </tr>
                <tr>
                  <td>OI</td>
                  <td>
                    ...
                    <?php foreach ($graduaciones['esfera'] as $value): 
                      $formattedValue = $value > 0 ? '+' . number_format($value, 2) : number_format($value, 2);
                      ?>
                      <option value="<?= number_format($value, 2) ?>" <?= $value == $data->oi_esfera ? 'selected' : '' ?>><?= $formattedValue ?></option>
                    <?php endforeach; ?>
                    </select>
                  </td>
                  <td>
                    ...
                    <?php foreach ($graduaciones['cilindro'] as $value): ?>
                      <option value="<?= number_format($value, 2) ?>" <?= $value == $data->oi_cilindro ? 'selected' : '' ?>><?= number_format($value, 2) ?></option>
                    <?php endforeach; ?>
                    </select>
                  </td>
                  <td>
                    ...
                    <?php foreach ($graduaciones['eje'] as $value): ?>
                      <option value="<?= number_format($value, 0) ?>" <?= $value == $data->oi_eje ? 'selected' : '' ?>><?= number_format($value, 0) ?>°</option>
                    <?php endforeach; ?>
                    </select>
                  </td>
                  <td class="col-adicion">
                    ...
                    <?php foreach ($graduaciones['add'] as $value): ?>
                      <option value="<?= number_format($value, 2) ?>" <?= $value == $data->oi_add ? 'selected' : '' ?>><?= number_format($value, 2) ?></option>
                    <?php endforeach; ?>
                    </select>
                  </td>
                  <td><input type="text" class="form-control" name="dnp_oi" value="<?=$data->oi_dnp?>"></td>
                  <td class="col-altura"><input type="text" class="form-control" name="altura_oi" value="<?=$data->oi_altura?>"></td>
                </tr>
              </table>
            </div>
            ...
            <div class="row mb-4">
              <div class="col-3">
                <button type="button" class="btn btn-primary" onclick="modalBootStrap('<?=$url("ecom/".$this->interfaz."/agregarProductoEdit?tipo=armazon")?>','', '60');">Agregar producto</button>
              </div>
              <div class="col-3">
                <button type="button" class="btn btn-primary" onclick="modalBootStrap('<?=$url("ecom/".$this->interfaz."/agregarProductoEdit?tipo=mica")?>','', '60');">Agregar mica</button>
              </div>
              <div class="col-3">
                <button type="button" class="btn btn-primary" onclick="modalBootStrap('<?=$url("ecom/".$this->interfaz."/agregarProductoEdit?tipo=extra")?>','', '60');">Agregar tratamiento</button>
              </div>
              <div class="col-3">
            ...
            <div class="row mt-4 mb-4">
              ...
              <textarea class="form-control" id="comentarios" rows="3"><?=$data->comentarios?></textarea>
            </div>
          ...
          <div class="col-3">
            ...
            <div class="form-group">
              ...
              <select class="form-control" id="sucursal" name="sucursal" disabled>
                ...
                <option value="<?=$value->id?>" <?=$value->id == $data->sucursal ? "selected":"";?>><?=$value->nombre?></option>
                <?}?>
              </select>
            </div>
            <div class="form-group">
              ...
              <select class="form-control" id="optometrista" name="optometrista">
                ...
                <option value="<?=$value->id?>" <?=$value->id == $data->optometrista ? "selected":"";?>><?=$value->nombre?> <?=$value->apaterno?> <?=$value->amaterno?></option>
                <?}?>
              </select>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
              ...
              <input type="number" class="form-control" id="descuento" value="<?=$data->descuento?>">
            </div>
            <div class="form-group">
              <label>Total</label>
            ...
            <div class="form-group">
              ...
              <input type="number" class="form-control" id="anticipo" value="<?=$data->anticipo?>">
            </div>
            <div class="form-group">
              <label>Forma de Pago</label>
              <select class="form-control" id="forma_pago">
                <option value="efectivo" <?=$data->forma_pago == "efectivo" ? "selected":"";?>>Efectivo</option>
                <option value="tarjeta" <?=$data->forma_pago == "tarjeta" ? "selected":"";?>>Tarjeta débito</option>
                <option value="tarjetac" <?=$data->forma_pago == "tarjetac" ? "selected":"";?>>Tarjeta crédito</option>
                <option value="vales" <?=$data->forma_pago == "vales" ? "selected":"";?>>Vales</option>
                <option value="transferencia" <?=$data->forma_pago == "transferencia" ? "selected":"";?>>Transferencia</option>
              </select>
            </div>
            <div class="form-group">
              <label>Saldo</label>
            ...
            <div class="form-group row">
              <div class="col-6 text-end">
                <button type="button" class="btn btn-primary w-100" onclick="guardarVenta('venta')">Realizar Venta</button>
              </div>
            </div>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    var tableData = $('#tabla-data').DataTable({
      ...
      "ajax": {
          "url": "<?=$url("ecom/".$this->interfaz."/getAllTemporalesEdit")?>",
          "dataSrc": ""
      },
      "columns": [
          {"data": "id"},
    ...
    $('#tabla-data tbody').on( 'click', 'button', function () {
      ...
      if(button.hasClass("boton-eliminar")){
        eliminaModel(data.id, "<?=$url("ecom/".$this->interfaz."/destroyTemporalEdit")?>", [], hDefault);
      }
    });
  ...
  function enviarSolicitudVenta(data, tipo) {
    ...
    modalBootStrapCallback(
      '<?=$url("ecom/".$this->interfaz."/confirmaVendedor")?>?sucursal=' + sucursal + '&vendedor=<?=$data->user;?>',
  ...
  function enviarSolicitudVentaS2(data, tipo) {
    $.post('<?=$url("ecom/".$this->interfaz."/saveCotizacionEdit")?>', data)
      .done(function(response) {
          ...
          if (response.error === undefined) {
              ...
              limpiarFormulario();
              var url = "<?=$url("ecom/".$this->interfaz."/ventas")?>";
