	...
	<div class="col-lg-12">
		...
		<?php if($conf->num_values != 1): ?>
			<input 
				...
				type="email" id="text<?=$field->id?>" 
				name="email<?=$field->id?>"
				data-email-msg="Introduce un email válido"/>
			<a id="btnAgregarEmail<?=$field->id?>">Agregar <?=$conf->label?></a>
			...
			<script>
				...
				$("#btnAgregarEmail<?=$field->id?>").kendoButton({
					click: function (e){
