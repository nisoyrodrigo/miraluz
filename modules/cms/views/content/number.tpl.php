...
<div class="row">
	<div class="col-lg-12">
		...
		<?php if($conf->num_values != 1): ?>
			<input type="text" id="number<?=$field->id?>"/>
			<script>
				$("#number<?=$field->id?>").kendoNumericTextBox({
					format: "0"
					<?=(!empty($conf->min_value) ? ",min: ".$conf->min_value: "")?>
					<?=(!empty($conf->max_value) ? ",max: ".$conf->max_value : "")?>
				});
			</script>
			<a id="btnAgregarNumero<?=$field->id?>">Agregar <?=$conf->label?></a>
			...
			<script>
				...
				$("#btnAgregarNumero<?=$field->id?>").kendoButton({
					click: function (e){
						if($("#number<?=$field->id?>").val().trim() == "" ){
							...
							$.getJSON(
								...
								{
									value: $("#number<?=$field->id?>").data("kendoNumericTextBox").value(),
									content: $("#id").val(),
									id_field: <?=$field->id?>
								},
								function(response){
									if(response.id == undefined){
										...
										$("#number<?=$field->id?>").data("kendoNumericTextBox").value("");
