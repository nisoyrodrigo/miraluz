	...
	<div class="col-lg-12">
				...
				$("#grid<?=$field->id?>").kendoGrid({
					...
					columns: [
						...
						},
						{
							field: "value_field",
							title: "&nbsp;",
							template: '<center><img src="<?=$url()?>#: data.value_field #" style="height:100px;border-radius:5px"/></center>',
							width: 250
						},
		...
		<?php else: ?>
			...
			<div style="width:30%;display:<?=(empty($value->value_field) ? "none" : "inline-block")?>" id="imagenVisible<?=$field->id?>">
				<center>
					<img src="<?=$url($value->value_field)?>" id="imagen-por-defecto<?=$field->id?>" style="height:80px;border-radius:5px" >
					<a href="javascript:eliminarImagen('<?=$field->id?>')" class="k-button">Eliminar</a>
				</center>
			</div>
			<script>
				$("#file_<?=$field->id?>").kendoUpload({
					...
					success: function (e) {
						...
						$("#imagen-por-defecto<?=$field->id?>").attr("src", "<?=$url()?>" + e.response.value_field);
					}
				});
			</script>
		<?php endif; ?>
