<?php 
	...
	if(!empty($conf->min_value)){
		$min = explode("-", $conf->min_value);
	}
	$max = "";
	if(!empty($conf->max_value)){
		$max = explode("-", $conf->max_value);
	}
?>
<div class="row">
	<div class="col-lg-12">
		...
		<?php if($conf->num_values != 1): ?>
			<input type="input" id="date<?=$field->id?>"/>
			<script>
				$("#date<?=$field->id?>").kendoDatePicker({
					format: "yyyy-MM-dd"
					<?=(!empty($conf->min_value) ? ",min: new Date(".$min[0].", ".($min[1]-1).", ".$min[2].")": "")?>
					<?=(!empty($conf->max_value) ? ",max: new Date(".$max[0].", ".($max[1]-1).", ".$max[2].")": "")?>
				});
			</script>
			<a id="btnAgregarFecha<?=$field->id?>">Agregar <?=$conf->label?></a>
			...
			<script>
				...
				$("#btnAgregarFecha<?=$field->id?>").kendoButton({
					click: function (e){
						if($("#date<?=$field->id?>").val().trim() == "" ){
